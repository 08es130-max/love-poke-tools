(() => {
  ...
  // mascot.js may be evaluated again after PWA navigation or script injection.
  // Claim the singleton immediately, before waiting for DOMContentLoaded.
  ...
  const SPRITE_URL = './assets/mascot/shioriko-sprite-fixed.png';
  ...
  const SPRITE = Object.freeze({
    ...
    columns: 4,
    rows: 5,
    rowIndex: Object.freeze({ right: 0, left: 1, down: 2, up: 3, idle: 4 }),
  ...
  const sizes = Object.freeze({ small: 68, medium: 88, large: 112 });
  ...
  function renderSprite(target, pose, frameNumber) {
    ...
    const scale = Number.parseFloat(target.style.getPropertyValue('--mascot-scale')) || sizes.medium / SPRITE.frameHeight;
    target.style.setProperty('--mascot-column', column);
    target.style.setProperty('--mascot-row', row);
    target.style.setProperty('--mascot-sheet-width', `${SPRITE.frameWidth * SPRITE.columns * scale}px`);
    target.style.setProperty('--mascot-sheet-height', `${SPRITE.frameHeight * SPRITE.rows * scale}px`);
    target.style.setProperty('--mascot-frame-x', `${-SPRITE.frameWidth * column * scale}px`);
    target.style.setProperty('--mascot-frame-y', `${-SPRITE.frameHeight * row * scale}px`);
  }
  ...
  function paint() {
    ...
    mascot.style.setProperty('--mascot-scale', box.height / SPRITE.frameHeight);
  ...
  function buildUi() {
    ...
    root.append(layer, settingsButton, dialog);
  ...
  function loadSprite() {
    ...
    image.onload = () => {
      imageReady = image.naturalWidth === SPRITE.frameWidth * SPRITE.columns && image.naturalHeight === SPRITE.frameHeight * SPRITE.rows;
      updateVisibility();
  ...
  function init() {
    // Keep an existing instance (and its active animation loop) instead of
    // creating another one. Also clean up malformed duplicate roots.
    if (removeDuplicateRoots()) return;
    buildUi();
    const box = dimensions();
    x = box.minX;
    y = box.maxY;
