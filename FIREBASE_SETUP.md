...
## 麻雀データ形式
`mahjongMatches/{対局ID}` は `schemaVersion: 1`、`id`、`date`、`type`、`createdAt`、`updatedAt` と `players` を保存します。各参加者は `playerId`、対局時点の `displayName`、`rank`、実際の持ち点 `finalPoints` を持ちます。将来の順位点は `scorePoint` 等の別フィールドとして追加できます。
