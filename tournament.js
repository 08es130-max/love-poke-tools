...
function renderSourceParticipants(){
  ...
  $('#tournamentSourcePlayers').innerHTML=players.map((player,index)=>`
    <div class="source-player">
      ...
      <span>${player.pokemon.length?player.pokemon.map(p=>escapeHtml(p.name)).join('・'):'ポケモン未抽選'}</span>
    </div>`).join('');
}
...
$('#startTournamentBtn').onclick=async()=>{
  ...
  const players=source.map(player=>({id:uid(),name:player.name,pokemon:player.pokemon}));
...
function renderHistory(){
  ...
  for(const tournament of completedTournaments){
    tournament.players.forEach(player=>{
      ...
      if(!careers.has(key))careers.set(key,{name:key,events:0,first:0,second:0,third:0,wins:0,losses:0,ranks:[]});
  ...
  select.innerHTML='<option value="">プレイヤーを選択</option>'+stats.map(s=>`<option value="${escapeHtml(s.name)}">${escapeHtml(s.name)}</option>`).join('');
  if(stats.some(s=>s.name===selected))select.value=selected;
  renderHeadToHead();
...
function renderHeadToHead(){
  ...
  for(const tournament of completedTournaments){
    const player=tournament.players.find(p=>p.name.trim()===selected);if(!player)continue;
    for(const match of tournament.matches){
      ...
      const opponent=playerById(tournament,match.a===player.id?match.b:match.a).name;
      if(!records.has(opponent))records.set(opponent,{leagueW:0,leagueL:0,suddenW:0,suddenL:0});
      const stat=records.get(opponent),won=match.winner===player.id,prefix=match.type==='league'?'league':'sudden';
      stat[`${prefix}${won?'W':'L'}`]++;
    }
  }
  $('#headToHeadStats').innerHTML=[...records.entries()].map(([name,s])=>{const wins=s.leagueW+s.suddenW,losses=s.leagueL+s.suddenL;return `<article class="career-card"><h3>${escapeHtml(selected)} vs ${escapeHtml(name)}</h3><div>通算 ${wins+losses}戦${wins}勝${losses}敗 ／ 勝率 ${percent(wins,wins+losses)}</div><div>通常リーグ ${s.leagueW}勝${s.leagueL}敗 ／ サドンデス ${s.suddenW}勝${s.suddenL}敗</div></article>`}).join('')||'<p class="note">対戦記録はありません。</p>';
}
...
async function initializeCloud(){
  ...
  const modules=await Promise.all([
    ...
    setCloudStatus('warn','読込エラー',`Firebase SDKを読み込めません：${error.message}`);
    return null;
  });
  if(!modules)return;
  const [{initializeApp},{getAuth,signInAnonymously},{getFirestore,doc,collection,onSnapshot,setDoc,deleteDoc,runTransaction}]=modules;
  try{
    const app=initializeApp(config),auth=getAuth(app);await signInAnonymously(auth);
    const db=getFirestore(app),activeRef=doc(db,...ACTIVE_PATH),historyRef=collection(db,'tournaments');
    cloud={
